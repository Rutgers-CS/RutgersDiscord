...
{
    /// <summary>
    /// Represents a base class for creating value selections based on messages.
    /// </summary>
    /// <typeparam name="TValue">The value type of the Selection</typeparam>
    public abstract class BaseMessageSelection<TValue>
    {
        /// <summary>
        /// Gets which users can interact with the <see cref="BaseMessageSelection{TValue}"/>.
        /// </summary>
        public IReadOnlyCollection<SocketUser> Users { get; }

        /// <summary>
        /// Determites whether everyone can interact with the <see cref="BaseMessageSelection{TValue}"/>.
        /// </summary>
        public bool IsUserRestricted => Users.Count > 0;

        /// <summary>
        /// Gets an ORing determiting what the <see cref="BaseMessageSelection{TValue}"/> will delete.
        /// </summary>
        public DeletionOptions Deletion { get; }

        protected BaseMessageSelection(IReadOnlyCollection<SocketUser> users,
        ...
        /// <summary>
        /// Checks whether or not a given <see cref="SocketMessage"/> is a valid input to the Selection.
        /// This is executed for every message by a allowed user.
        /// </summary>
        /// <param name="message">The message to check</param>
        /// <returns></returns>
        protected abstract bool ShouldProcess(SocketMessage message);

        /// <summary>
        /// Parses the <see cref="SocketMessage"/> to the related <see cref="TValue"/>.
        /// This is executed after <see cref="ShouldProcess(SocketMessage)"/> returned true.
        /// </summary>
        /// <param name="message">The message to parse</param>
        /// <returns></returns>
        protected abstract Optional<TValue> ParseValue(SocketMessage message);
        ...
        /// <returns></returns>
        protected virtual Task InitializeMessageAsync(IUserMessage message)
            => Task.CompletedTask;

        /// <summary>
        /// Modifies the message after the selection has exited.
        /// This should be used to set the content to the result etc.
        /// </summary>
        ...
        internal bool InternalShouldProcess(SocketMessage message)
            => (!IsUserRestricted || Users.Any(x => x.Id == message.Author.Id)) && ShouldProcess(message);

        internal Optional<TValue> InternalParseValue(SocketMessage message)
            => ParseValue(message);
