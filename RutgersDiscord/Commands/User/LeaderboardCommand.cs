...
namespace RutgersDiscord.Commands.User
{
    ...
    public class LeaderboardCommand
    {
        ...
        private List<LeaderboardTeamData> SortBy(List<LeaderboardTeamData> teamsdata, string input)
        {
            if(input == "record")
            {
                return teamsdata.OrderByDescending(x => x.Win).ThenByDescending(x => x.WLDiff).ThenByDescending(x => x.RoundDiff).ThenByDescending(x => x.KD).ToList();
            } 
            else if (input == "kd") 
            {
            ...
            {
                return teamsdata.OrderByDescending(x => x.RoundDiff).ThenByDescending(x => x.Win).ThenByDescending(x => x.WLDiff).ThenByDescending(x => x.KD).ToList();
            }
            else 
            {
                return teamsdata.OrderByDescending(x => x.Win).ThenByDescending(x => x.WLDiff).ThenByDescending(x => x.RoundDiff).ThenByDescending(x => x.KD).ToList();
