    ...
    public class HelpCommand
    {
        ...
        public async Task GetHelp()
        {
         ...
         // /reschedule  and // /veto ------ only works in match channel, only by team captain
            ...
            EmbedBuilder embed = new EmbedBuilder().WithTitle("Rutgers CS:GO Commands").WithColor(new Color(102, 0, 0)).AddField("General Commands", $"```{gencommands}```", true).AddField("Match Room Commands", $"```{eventcommands}```", false).WithThumbnailUrl("https://i.imgur.com/hePqR1R.png");
